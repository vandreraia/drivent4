...
import supertest from "supertest"; import {
  ...
  createRoomWithHotelIdAndCapacity1,
} from "../factories";
...
describe("POST /booking", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 403 when capacity is full", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
...
describe("PUT /booking", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 403 when capacity is full", async () => {
      ...
      const response = await server.put(`/booking/${newroom.id}`).set("Authorization", `Bearer ${token}`).send(body);
    ...
    it("should respond with status 403 when user doenst have a booking", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
